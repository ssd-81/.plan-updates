...
## 27-07-2025
- Pokedex CLI project continued
	- solved the integration of cache in main code 
...
## 28-07-2025
- Rust 
	- installed rust on system
	- ran a few programs 
	- cargo run 
	- realized that rust is quite strict with its type system 
	- strings are tricky to deal with
		- not really important for now, but the reason behind that is the dynamic size and the way strings access the memory (stack/heap model)
		- strings change length at runtime. 
> timestamp: 1:02:00
- Pokedex CLI project
	- reading PokeAPI documentation to find which endpoint to call to get all the pokemons located in an area
	- using curl command to make test calls to different endpoints to check their response 
	- learning how to craft struct for unmarshalling responses from the server 
> I have got some momentum and things are getting intersting; I need to get to zero to one. For me, getting zero to one would be to actually contribute to some real world production codebase or work with professionals. I am in this for the long haul, but that doesn't mean I can slack, rather it is the signal that I need to build substance above all else. Let's get going. 
...
## 29-07-2025
- Pokedex CLI project
	- I need to create a struct(s) for handling unmarshalling of response from an API endpoint; continuing on that. 
	- changed function signatures for all command callback functions to take up arguments for command 
> Starting to gain some some momentum. Right now at level 0, need to level to 1. 
> Benchmarking: 7-8 hours of consistent deep work for 69 days => level 1


--- 

## 30-07-2025
- Pokedex CLI projects
	- solved bug in code preventing successive caching and API calling
	- started working on catch command to catch pokemons and add them to pokedex map once caught
		- defined struct for storing pokemon data from the API repsonse 
...
## 31-07-2025
- Pokedex CLI project
	- working on using randomness (pseudo) to create a pokemon catching mechanic based on the baseExperience of each pokemon
	- added inspect command to check for the caught pokemon in the pokedex struct 
	- added catch command to catch a pokemon using baseExperiene as a difficulty measure. 
 
- SQL 
	- SQL databases and their basic structures
	- Basic queries
	- SQL vs NoSQL
	- Interesting ideas to explore further
		- SQLite is a serverless DBMS ( has the ability to run within applications) 
		- PostgreSQL uses Client-Server model and requires a server to be installed and listening on a network
	- SQLite doesn't enforce types ; but that doesn't mean it is fine. 
